...
import pg from 'pg'; // Import node-postgres directly if needed by pgvector lib
import { Pool } from 'pg'; // Import Pool type
import pgvector from 'pgvector/pg'; // Import the pgvector library
import { openai } from '@ai-sdk/openai';
import { env } from '../../utils/env.js';
import { pool } from '../../db/index.js'; // Import the postgres pool
...
async function registerVectorType(poolInstance: Pool) {
  const client = await poolInstance.connect();
  try {
    await pgvector.registerType(client); // Use the client from the pool
    console.log('pgvector type registered successfully.');
  } finally {
    client.release();
  }
}
registerVectorType(pool).catch(err => console.error('Failed to register pgvector type:', err));
...
// Interaction with the vector store will happen through direct SQL queries using the `pool` and pgvector functions.
...
export async function initializeVectorStoreTable() {
  try {
    ...
    if (!tableExistsRes.rows[0].exists) {
      await pool.query(`CREATE TABLE ${env.PGVECTOR_TABLE_NAME} (
      ...
      // await pool.query(`CREATE INDEX ON ${env.PGVECTOR_TABLE_NAME} USING hnsw (${env.PGVECTOR_VECTOR_COLUMN} vector_l2_ops);`);
